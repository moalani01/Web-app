import type { MessageType, FeedbackMessage } from '@/types/iot'
...
const SIMULATION_SCENARIOS: SimulationScenario[] = [
  ...
  { 
    ...
    details: 'Invalid sensor type for current firmware', 
    probability: 0.1 
  },
  { 
    type: 'error', 
  ...
  { 
    type: 'info', 
    message: 'Configuration queued for processing', 
    details: 'Device is currently busy, will apply when available', 
    probability: 0.1 
  },
  { 
    type: 'info', 
...
export const simulateZmqCommunication = async (
  messageType: MessageType,
  formData: Record<string, any>
): Promise<Omit<FeedbackMessage, 'id' | 'timestamp'>> => {
  // Simulate network delay
  await randomDelay(CONFIG.SIMULATION.MIN_DELAY, CONFIG.SIMULATION.MAX_DELAY)
  ...
  return {
    type: SIMULATION_SCENARIOS[0].type,
    message: SIMULATION_SCENARIOS[0].message,
    details: SIMULATION_SCENARIOS[0].details
  }
}